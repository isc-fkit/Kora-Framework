...
def main():
    ...
    try:
        ...
        if gate_ok and not is_user_pkg and not args.dry_run:
            ...
            # --roles-confirmed (qua cổng chặn VAI TRÒ) + --ai-confirmed (qua CỔNG CHẶN PHÂN TÍCH — nền headless
            # KHÔNG spawn sub-agent được; phân tích AI chèn riêng qua --inject-ai ở bước sau nếu có).
            rep_args = ["--source-ids", "all", "--roles-confirmed", "--ai-confirmed"]
