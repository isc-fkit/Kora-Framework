#!/usr/bin/env python3
"""
sync_gitlab.py — Đẩy Knowledge Base → repo GitLab RIÊNG TƯ (private) qua GIT PUSH.

Anh em với tools/github-sync & tools/confluence-sync: cùng triết lý "gom tri thức cục bộ → đẩy
lên 1 nơi chung", target là 1 git repo GitLab. Đường HEADLESS (chạy được trong cron/scheduler).

Bảo mật:
  - PAT đọc từ tools/gitlab-sync/.env.local (KORA_GITLAB_SYNC_TOKEN) — đã gitignore.
  - Token KHÔNG vào .git/config, KHÔNG vào argv/ps: bơm qua GIT_CONFIG_* (env) dưới dạng
    http.extraHeader (Authorization: Basic base64("oauth2:"+token)). Output git được "scrub".
  - Repo đích là 1 GƯƠNG (mirror sink) 1 chiều: mỗi push reset cứng worktree theo origin rồi
    chép lại note → lịch sử git là vết kiểm toán.

Chỉ thư viện chuẩn Python 3 + lệnh `git`. Tái dùng helper của sync_confluence.

Ví dụ:
  python3 tools/gitlab-sync/sync_gitlab.py --check
  python3 tools/gitlab-sync/sync_gitlab.py --push --dry-run
  python3 tools/gitlab-sync/sync_gitlab.py --push
  python3 tools/gitlab-sync/sync_gitlab.py --pull
...
WORK_DIR = HERE / "work"
TOKEN_KEYS = ("KORA_GITLAB_SYNC_TOKEN", "KORA_GITLAB_TOKEN", "GITLAB_TOKEN")
...
def scrub(text: str, token: str) -> str:
    ...
    if token:
        ...
        try:
            b64 = base64.b64encode(("oauth2:" + token).encode()).decode()
...
def gl_cfg(cfg: dict, args) -> dict:
    repo = (getattr(args, "repo", None) or cfg.get("gitlab.repo") or "").strip().strip("/")
    if "/" not in repo:
        die("Thiếu repo đích. Đặt gitlab.repo = \"group/name\" (nhóm con: \"group/sub/name\") "
            "trong config hoặc truyền --repo.")
    base_url = (cfg.get("gitlab.base_url") or "https://gitlab.com").rstrip("/")
    return {
        "repo": repo,
        "owner": repo.split("/")[0],
        "name": repo.split("/", 1)[1],           # giữ phần còn lại (hỗ trợ nhóm con)
        "branch": getattr(args, "branch", None) or cfg.get("gitlab.branch") or "main",
        "base_url": base_url,
        "url": f"{base_url}/{repo}.git",
        "permission": (cfg.get("gitlab.permission") or "read_write").lower(),
        "source": getattr(args, "source", None) or cfg.get("gitlab.push.source") or "both",
        "scope": getattr(args, "scope", None) or cfg.get("gitlab.push.scope") or "",
        "subdir": (getattr(args, "subdir", None) or cfg.get("gitlab.push.subdir") or "").strip("/"),
        "slug": repo.replace("/", "-"),
    }
...
def git_env(token: str) -> dict:
    ...
    GitLab PAT qua HTTPS: Authorization Basic base64("oauth2:"+token)."""
    e = dict(os.environ)
    ...
    if token:
        hdr = "Authorization: Basic " + base64.b64encode(("oauth2:" + token).encode()).decode()
...
def gl_sysdir(cfg: dict) -> Path:
    d = cs.vault_dir(cfg) / "_system" / "gitlab"
    d.mkdir(parents=True, exist_ok=True)
    return d


def map_path(cfg, g) -> Path:
    return gl_sysdir(cfg) / f"gitlab-map-{g['slug']}.json"
...
def cmd_check(cfg, g, token):
    if not token:
        die("Chưa có token. Tạo tools/gitlab-sync/.env.local (copy .env.example) và điền "
            "KORA_GITLAB_SYNC_TOKEN = PAT (scope 'api' hoặc 'read_repository'+'write_repository').")
    ...
    print(f"✅ Kết nối GitLab OK — repo {g['repo']} ({nbranch} nhánh) — branch đích: {g['branch']}")
    return 0


def cmd_push(cfg, g, token, dry_run=False, force=False):
    if g["permission"] == "read_only":
        die("gitlab.permission = read_only → KHÔNG được đẩy (push).")
    if not token and not dry_run:
        die("Chưa có token. Điền KORA_GITLAB_SYNC_TOKEN trong tools/gitlab-sync/.env.local.")
...
# ───────── chuyển file GitLab → document chuẩn wiki (frontmatter + link nguồn) ─────────
...
def _build_gitlab_index(dest_root: Path):
    """Dựng lại trang hub _GitLab-Index.md từ MỌI document GitLab đã kéo (idempotent)."""
    ...
    for p in sorted(dest_root.rglob("*.md")):
        ...
        if fm.get("source") != "gitlab":
            continue
        repo = fm.get("gitlab_repo", "?")
        rel = p.relative_to(dest_root).as_posix()
        groups.setdefault(repo, []).append(
            (fm.get("title") or p.stem, rel, fm.get("gitlab_url", "")))
    out = [_emit_fm({"title": "GitLab — Index", "source": "gitlab",
                     "type": "index", "generated": "true", "imported_at": cs.now_iso()}),
           "# GitLab — Index tri thức", "",
           "> Trang điều hướng TỰ SINH từ tài liệu kéo về từ GitLab (mỗi lần `--pull`). "
    ...
    for repo in sorted(groups):
        ...
        total += len(items)
    out.append("---")
    out.append(f"*Tổng {total} tài liệu GitLab.*")
    (dest_root / "_GitLab-Index.md").write_text("\n".join(out) + "\n", encoding="utf-8")
...
def cmd_pull(cfg, g, token, dry_run=False):
    ...
    dest_root = cs.vault_dir(cfg) / "GitLab"
    ...
    if dry_run:
        ...
        print(f"  [dry] {len(md)} file → {repo_dir} (+ frontmatter, link nguồn, _GitLab-Index.md)")
        return 0
    ...
    for p in md:
        ...
        gl_path = ((g["subdir"] + "/") if g["subdir"] else "") + rel.as_posix()
        gl_url = f"{base}/{repo}/-/blob/{branch}/{urllib.parse.quote(gl_path)}"   # GitLab blob URL: /-/blob/
        ...
        for k, v in ofm.items():                    # giữ key gốc của repo (không đè key của ta)
            if k not in meta:
                meta[k] = v
        link = f"> 📎 Nguồn GitLab: [{repo}/{gl_path}]({gl_url})\n\n"
        out = repo_dir / rel
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(_emit_fm(meta) + "\n" + link + body.lstrip("\n"), encoding="utf-8")
        n += 1
    _build_gitlab_index(dest_root)
    print(f"✅ Pull xong: {n} file (đã thêm frontmatter + link nguồn) → {repo_dir}; cập nhật _GitLab-Index.md")
    return 0


def main():
    ap = argparse.ArgumentParser(description="Đẩy KB → repo GitLab riêng tư (git push).")
    ...
    ap.add_argument("--pull", action="store_true", help="Kéo file repo → <vault>/GitLab/.")
    ...
    ap.add_argument("--repo", help="group/name (mặc định gitlab.repo).")
    ap.add_argument("--branch", help="Branch đích (mặc định gitlab.branch).")
    args = ap.parse_args()
