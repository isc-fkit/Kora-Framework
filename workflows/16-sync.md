...
- SharePoint: `sharepoint.enabled: true`, `base_url`, `site_name`, `library`.
...
2. Chọn 1 trong 2 cách auth:
   - **App-only (chạy NỀN được):** tạo **Client secret**; cấp **Application permission `Sites.ReadWrite.All`** +
     **admin consent**. Đặt env `SHAREPOINT_TENANT_ID` / `SHAREPOINT_CLIENT_ID` / `SHAREPOINT_CLIENT_SECRET`
     (`~/.zshrc`; hoặc `tools/sharepoint-sync/.env.local` cho lịch nền).
   - **Device-flow (tương tác):** cấp **Delegated `Sites.ReadWrite.All`**; chạy `python3 tools/sharepoint-sync/sync_sharepoint.py --login`.
3. Config: `sharepoint.base_url: "https://<tenant>.sharepoint.com"`, `site_name`, `library`, `enabled: true`.
4. `python3 tools/sharepoint-sync/sync_sharepoint.py --check`.
`sharepoint.permission: read_only` → `--push` bị từ chối (chỉ `--pull`). ⚠️ Sandbox Cowork chặn API →
