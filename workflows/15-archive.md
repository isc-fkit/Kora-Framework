...
3b. **Email TICKET SỰ CỐ cho gói USER (áp dụng sẵn).** Cấu hình `scheduler.error_recipients` (NGƯỜI PHỤ TRÁCH)
   ...
   `/claude-knowledge-alert-mail` hoặc `/claude-knowledge-schedule` Bước 3B §5). TRƯỚC khi đóng gói:
...
4. ✋ **Confirm → chạy** `scripts/archive-kb.command` (Windows `scripts\archive-kb.bat`) với các biến
   trên. Tạo `kora-archive-<project>-<date>.zip` = thư mục `claude-knowledge-archive/` {manifest, data/, .env.local
...
5. Gửi file zip cho user (kèm đường dẫn tuyệt đối để copy). Key **WRITE** (nếu user được quyền push)
   cấp NGOÀI luồng (env `KORA_CLOUD_WRITE_*` hoặc `/claude-knowledge-connect`), KHÔNG bỏ vào gói.
...
1. Chép zip vào gốc repo → chạy `scripts/import-kb.command` (Windows `scripts\import-kb.bat`).
   ...
   tạo marker `.claude-knowledge-user` nếu là gói USER, dựng lại index.
...
2. **Sau import (Claude làm, có Approval Gate):**
   ...
   - **TẮT report/mail:** vì có `.claude-knowledge-user`, các luồng báo cáo/gửi mail (WF14, WF08 Mục B, `/claude-knowledge-daily-report`)
