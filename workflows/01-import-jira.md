...
> 🎯 **Lưu ý — ƯU TIÊN MCP (xử lý ở tầng trên `/claude-knowledge-scan`):** với "quét jira" CHUNG, nếu có nguồn Jira
> qua **MCP** (Atlassian Cloud/Rovo) thì quét THẲNG qua MCP, **KHÔNG vào Bước 0 này, KHÔNG hỏi domain**. Bước 0
> (chọn `.env` nguồn API/token bên dưới) CHỈ áp khi: dùng nguồn **API/Server self-host**, HOẶC user nêu rõ
> "quét jira server / nguồn nội bộ", HOẶC không có nguồn MCP nào.

Hỗ trợ **nhiều nguồn** (Server tự host + Cloud Atlassian), mỗi nguồn một file cấu hình
`tools/jira-to-obsidian/.env.<tên>` (mặc định `.env.local`). TRƯỚC khi quét (nguồn API), để user CHỌN nguồn:
...
> Nhờ bước này, khi dùng **nguồn API/token**, "quét jira" cho chọn quét từ domain nào (nội bộ / Atlassian Cloud qua
> token), không khóa cứng một nguồn. (Nguồn **MCP** được ưu tiên ở tầng trên — quét thẳng, không qua bước chọn này.)
