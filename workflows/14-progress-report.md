...
- **`method: smtp`** (full-auto, hợp lịch nền) — chạy (Claude tự chạy trong sandbox; user chạy tay thì Windows `py`).
  ...
  > 🖥️ **Cowork sandbox chặn SMTP → BÀN GIAO TERMINAL (không dead-end):** đọc stderr — nếu có **`SMTP_UNREACHABLE`**
  > (mạng bị chặn): báo cáo ĐÃ ở `reports/` (local thật) → chạy lại CÙNG lệnh + `--emit-command` (in 1 dòng lệnh path
  > tuyệt đối, KHÔNG gửi) → ghi `reports/kora-send-mail.command` (`#!/bin/bash` + `chmod +x`; Windows `.bat`) → báo user
  > mở **Terminal** chạy `bash "reports/kora-send-mail.command"` để **gửi tiếp việc dang dở** (terminal gửi SMTP được,
