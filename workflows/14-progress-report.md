...
- **`method: smtp`** (full-auto, hợp lịch nền) — chạy (Claude tự chạy trong sandbox; user chạy tay thì Windows `py`).
  ...
  > 🏷️ **Tiêu đề LUÔN có prefix `[Kora]`** — `send_report.py` tự ép nếu `--subject` thiếu (env `KORA_SUBJECT_PREFIX`, mặc định `[Kora]`, không nhân đôi). Nên đặt `reports.email.subject` = `"[Kora] Báo cáo <loại> <project> {date}"`.
