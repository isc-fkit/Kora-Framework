...
1. **Chọn TARGET** (AskUserQuestion, multi-select): **[Confluence] / [GitHub] / [SharePoint]** — chỉ hiện
   target đã bật (`confluence.enabled` / `github.enabled` / `sharepoint.enabled`). Chưa cấu hình → đi mục "Cấu hình" trong WF16 trước.
