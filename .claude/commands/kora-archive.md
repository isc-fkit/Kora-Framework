...
The user invoked `/kora-archive` — đóng gói KB có phân quyền + mật khẩu để bàn giao. Theo
`workflows/15-archive.md`. Khác `/kora-export-knowledge-base` (sao lưu thuần, không cổng/không phân quyền).
...
**Người nhận:** dùng `/kora-export-knowledge-base`? Không — dùng `scripts/import-kb.command` (đã hỗ trợ
gói archive: đặt key READ, tạo `.kora-user`, tắt report/mail, gợi ý lên lịch get&post). Xem WF15 mục B.
...
1. **HOST:** trước/khi archive, `/kora-sync` đẩy KB lên **GitHub private** và/hoặc **Confluence chung**
   (cổng `KORA_OPS_PW`). Gói archive USER ship **key READ** + `.kora-user` (tắt report/mail).
2. **USER** (máy base sạch): mở **Claude Desktop** → tạo project → **import source host đã export**
   ...
   Confluence của host → mở **/kora-schedule** (hoặc lịch Cowork) tạo lịch **kéo (pull) đồng bộ** → đúng
