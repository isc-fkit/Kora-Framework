...
3–4. **VỚI MỖI nguồn đã chọn → quét bằng ROUTE RIÊNG (vòng lặp), tích lũy vào CÙNG vault** (đọc `method`/`source_type`/
   ...
   - **`source_type: sheet`/`excel` (method `mcp`) — Excel trên SharePoint 365**: ĐỊNH VỊ bằng MCP, TẢI bằng Graph (đáng tin nhất). `sharepoint_search` `query="<tên file>" fileType="xlsx"` → lấy URI `file:///{driveId}/{itemId}` → tách driveId+itemId → `python3 "$T/excel-to-obsidian/import_excel.py" --graph-item "<driveId>/<itemId>" [--sheet <ten>] --source-id <id> [--project <KEY>] [--map …]` (xin Graph token từ creds `SHAREPOINT_*` app **Sites.Read.All** → `/drives/.../content` → parse ô chuẩn, honor HTTPS_PROXY). ⚠️ KHÔNG dùng `read_resource` lấy ô (trả text lệch cột, không downloadUrl). **Fallback** không có creds → user tải file → `--file`. **Google Sheet**: Publish-CSV → `--from-url "<csv_url>"`. (App-only Sites.Read.All → chạy được cả nền; device-flow → chỉ tương tác.)
