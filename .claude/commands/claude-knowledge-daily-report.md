...
3–4. **VỚI MỖI nguồn đã chọn → quét bằng ROUTE RIÊNG (vòng lặp), tích lũy vào CÙNG vault** (đọc `method`/`source_type`/
   ...
   - **`source_type: sheet`/`excel` (method `mcp`) — Excel trên SharePoint 365** (connector **Microsoft 365** đã connected): `sharepoint_search` `query="<tên file>" fileType="xlsx"` → `read_resource` URI file → lấy **`@microsoft.graph.downloadUrl`** → `python3 "$T/excel-to-obsidian/import_excel.py" --from-url "<downloadUrl>" [--sheet <ten>] --source-id <id> [--project <KEY>] [--map …]` (tool TẢI .xlsx thật + parse ô chuẩn, honor HTTPS_PROXY). **Fallback** không có downloadUrl → read_resource text → Claude chuẩn hoá `reports/_sheet-<id>.csv` → `--from-rows`. **Google Sheet**: Publish-to-web CSV → `--from-url "<csv_url>"`. (Token connector do app giữ → **chỉ TƯƠNG TÁC**, không chạy nền.)
