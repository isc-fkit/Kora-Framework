...
> ⚠️ **SKILL KHỚP NHƯNG KHÔNG GỌI ĐƯỢC — ÁP CHO MỌI SKILL, KHÔNG BAO GIỜ DEAD-END.** 2 trường hợp xử lý NHƯ NHAU:
> **(a)** skill KHÔNG có trong available list (Cowork chưa upload); **(b)** gọi `Skill` mà tool báo **`Failed`/not
> found/bị từ chối**. → **NGAY TRONG LƯỢT ĐÓ tự thực hiện việc tương đương — KHÔNG dừng ở lời xin lỗi, KHÔNG hỏi ngược
> user** ("framework nào?", "bạn muốn gì?"), KHÔNG bắt user gõ lại. **ĐƯỜNG DỰ PHÒNG PHỔ QUÁT (đủ cho CẢ 22 skill):**
> ① skill có **workflow tương đương** → đọc + chạy workflow đó (map: init=00 · scan=01/01b · import file=02 · export
> docs=06 · schedule=08 · evolve KB=09 · **update=10** · export/import KB=11 · release=12 · evolve hệ thống=13 ·
> **daily-report=14** · archive=15 · sync=16 · canva=17 · campaign=18 · worklog-check=19 · GEO=20; project không có
> `workflows/` → đọc `~/.claude/kora-framework/workflows/`); ② skill KHÔNG có workflow riêng (connect · send-mail ·
> ops-password · alert-mail · version · uninstall…) → **MỞ FILE SKILL TRÊN ĐĨA và LÀM THEO NỘI DUNG như thể skill đã
> nạp**: thứ tự tìm `<project>/Skill/claude-knowledge-<x>.md` → `~/.claude/commands/` → `~/.claude/kora-framework/Skill/`.
> File skill LUÔN ship kèm mọi bản cài (installer/update tự refresh cả 3 nơi) nên đường ② KHÔNG BAO GIỜ thiếu. Lệnh
> chạy qua `run_command`/Bash/MCP theo §⚡; **vẫn confirm 1 câu cho skill gated/outward** + cổng `KORA_OPS_PW` giữ nguyên.
> **Đặc biệt "cập nhật"**: chính lúc cần update là lúc skill hay thiếu/cũ → chạy thẳng `workflows/10-update.md`. Lỗi
> thật đã gặp: user gõ *"Cập nhật phiên bản mới nhất framework"* → `Skill claude-knowledge-update` Failed → model hỏi
> ngược thay vì chạy WF10 — **SAI**. Xử lý xong, NHẮC user 1 dòng: *"skill trong app thiếu/cũ — upload lại các file
> trong `Skill/` vào mục Skills để lần sau kích hoạt trực tiếp."*
...
| "quét jira" / "lấy dữ liệu mới (từ) jira" / "quét dữ liệu" / **"cập nhật dữ liệu (mới) (từ) jira"** (toàn bộ project) | Confirm → **HỎI NGUỒN nếu có ≥2 nguồn Jira** (để user DỄ CHỌN): có nhiều nguồn `jira_*` (vd Atlassian Cloud (MCP) + Jira Server host (API)) → **AskUserQuestion** liệt kê từng nguồn (kèm MCP/API) + **[Cả 2 nguồn]** cho user chọn. **Đúng 1 nguồn** → dùng luôn (không hỏi). User nêu rõ nguồn trong câu ("jira server"/"nội bộ"/"cả 2") → theo đó. Sau khi chọn: **MCP** quét THẲNG **rồi BẮT BUỘC GHI VAULT qua `import_jira.py --from-mcp <file>`** (đừng chỉ đọc inline → mất tri thức); **API/Server** → `workflows/01-import-jira.md` (Bước 0 chọn `.env`). **Chọn project → liệt kê ĐẦY ĐỦ từng project + PREFIX nguồn** (vd `[Cloud·MCP] FA — …`), >4 → phân trang. *(Khác "cập nhật phiên bản/ứng dụng" = WF10 update APP.)* **Cowork (sandbox chặn mạng):** nguồn **MCP** quét THẲNG trong chat; nguồn **API** in marker `NETWORK_UNREACHABLE` → **(a) có MCP `run_command` (local-terminal, Claude Desktop) → gọi nó chạy `import_jira.py` THẲNG trên máy (ngoài sandbox), KHÔNG bàn giao;** (b) không có → **BÀN GIAO**: ghi sẵn `reports/claude-knowledge-scan.command` (`import_jira.py --emit-command`, token KHÔNG in) cho user chạy `bash reports/claude-knowledge-scan.command` ở **Terminal**. Terminal CLI: quét thẳng. |
| "quét task <KEY>" / "quét epic <KEY>" (vd `quét task PROJ-102`) | Confirm → chạy `workflows/01b-import-jira-single.md` |
| "kết nối nguồn", "connect", "thêm Jira/GitHub/GitLab/Confluence/SharePoint/Gmail" | Confirm → `/claude-knowledge-connect`: chọn **MCP/API** → nguồn (API ưu tiên **OAuth 2.0**; API vs MCP tính RIÊNG) → verify → ghi `connections:`. **Gmail → LUÔN hỏi thẳng "SMTP hay MCP", ƯU TIÊN SMTP** (tự động gửi); chỉ fallback MCP (nháp) khi user chọn rõ / không lập được App Password. **Mạng chặn SMTP → thêm [Gmail API (OAuth2)]:** flow TỰ TẠO file input tạm `tools/report-mailer/.oauth-input` (placeholder `GMAIL_OAUTH_CLIENT_ID/SECRET` + `KORA_HTTPS_PROXY`) → user **dán 2 giá trị vào FILE** (KHÔNG qua chat) → `gmail_oauth_setup.py --env <file> --write-zshrc` (đọc creds từ file → browser uỷ quyền → **GHI `GMAIL_OAUTH_*` (+ `KORA_HTTPS_PROXY`) vào `~/.zshrc`** theo rule #6, KHÔNG in token) → **xoá file input** → `send_report.py --transport auto` tự fallback SMTP→Gmail API. Proxy RIÊNG `KORA_HTTPS_PROXY` (không đụng `HTTPS_PROXY` hệ thống/toggle). *(Lịch nền → `--write-env .env.local`.)* **Ghi token vào `~/.zshrc` → LUÔN `source ~/.zshrc` để lấy config MỚI** (run_command tự source mỗi lần). |
...
| "báo cáo tiến độ", "report tiến độ", "tiến độ dự án", "cập nhật tiến độ", "sinh báo cáo" (kể cả khi **PM/PO hỏi bằng lời** trong Cowork) | Confirm → **① LUÔN HỎI LOẠI BÁO CÁO TRƯỚC TIÊN** (AskUserQuestion 5 loại như `/claude-knowledge-daily-report` Bước 1b: Tiến độ · Cuộc họp · Tiến độ+Meeting+Roadmap/OKR · Tài chính/hoá đơn · Custom — >4 → phân trang; loại ≠ Tiến độ → đi ĐÚNG nhánh loại đó; **MỖI loại BẮT BUỘC spawn sub-agent chuyên biệt phân tích — code-gate `build_report.py`**) → **② HỎI NGUỒN — liệt kê ĐỘNG ĐẦY ĐỦ mọi nguồn ĐANG kết nối (MCP + API)** (AskUserQuestion multiSelect dựng từ `check_connection.py --list --json` + dò MCP: mỗi instance Jira 1 mục `[Jira·API|MCP] host` · `[SharePoint]` · `[Google Sheet (Composio)]` · `[Local Excel]`; đúng 1 nguồn → khỏi hỏi) → **③ HỎI DỰ ÁN NÀO** (liệt kê **ĐẦY ĐỦ từng project + PREFIX nguồn** vd `[Cloud·MCP] FA — …`, >4 phân trang: API `--list-projects` / MCP `getVisibleJiraProjects`; multi-select) → **CỔNG MẬT KHẨU `KORA_OPS_PW`** (sai → DỪNG) → chạy `workflows/14-progress-report.md`: **tự LÀM MỚI — kéo các mục MỚI/cập nhật** (mốc "mới" = `updated >= last-import-<nguồn>`, riêng theo nguồn) **của (các) project đã chọn** (Cloud `*.atlassian.net`/MCP → kéo về vault + reindex; self-host → kiểm tra độ mới; **báo rõ "đang lấy dữ liệu cập nhật từ <mốc>"**) → phân tích AI (sub-agent — **model `sonnet`** cho ops/kế toán/thư ký/bám-template (nhanh, song song), **`opus`** cho chiến lược-PM/GEO (suy luận sâu); môi trường không hỗ trợ tham số `model` → kế thừa model phiên) → sinh dashboard → **UI inline Cowork** + file HTML → **④ đề xuất gửi mail** ([Gửi mail ngay]/[Dừng]) → **⑤ [Gửi ngay] → HỎI NGƯỜI NHẬN (NHIỀU email, phẩy — list lưu per-project) + TIÊU ĐỀ (gợi ý động `reports/_subject-latest.txt` + ô "Other") → TỰ GỬI** → **⑥ NGAY SAU GỬI XONG BẮT BUỘC hỏi ĐẶT LỊCH tự động** ([Đặt lịch hằng ngày]/[Thôi] → `schedule.py register … --email "<list vừa gửi>"`). Gửi: tự dùng **Gmail SMTP nếu đã setup**; **Cowork chặn SMTP → (a) có MCP `run_command` (local-terminal, Claude Desktop) → gọi nó chạy `send_report.py` GỬI THẲNG trên máy, KHÔNG bàn giao;** (b) không có → **BÀN GIAO: xuất lệnh bash (`reports/claude-knowledge-send-mail.command`) cho user chạy ở TERMINAL gửi tiếp** (report đã build sẵn ở local — terminal chỉ gửi). Terminal CLI: SMTP gửi thẳng. |
...
| "cập nhật phiên bản", "cập nhật phiên bản mới nhất", "cập nhật ứng dụng / app", **"cập nhật framework"**, "update framework", "nâng cấp Kora / hệ thống", "lên bản mới nhất", "có bản mới không", "kiểm tra phiên bản" | **= Cập nhật CHƯƠNG TRÌNH (app) lên bản phát hành mới nhất** → chạy `workflows/10-update.md` (giữ nguyên tri thức). 🏷️ **"framework" / "Kora" / "hệ thống" / "app" / "chương trình" = bí danh của CHÍNH ứng dụng này — TUYỆT ĐỐI KHÔNG hỏi lại "framework nào? đang ở đâu?"** (lỗi thật đã gặp: user gõ *"cập nhật phiên bản mới nhất framework"* bị hỏi ngược → SAI; cứ chạy WF10). **CHẠY THẲNG WF10 — KHÔNG phụ thuộc skill `/claude-knowledge-update` có trong available list hay không;** skill không có → ĐỪNG thử `Skill …` rồi "xin lỗi gọi nhầm", đi luôn WF10 qua `run_command` (xem §🎯). **TUYỆT ĐỐI KHÔNG** hỏi lại "bạn muốn cập nhật cái gì" — chạy thẳng WF10 (WF10 tự confirm trước khi tải/ghi đè). **CORE ở `~/.claude/kora-framework` (ngoài sandbox) → KHÔNG tải/ghi đè từ trong chat.** WF10: **(a) có MCP `run_command` (local-terminal, Claude Desktop) → chạy lệnh cập nhật THẲNG trên máy, KHÔNG bắt user mở Terminal;** (b) không có → BÀN GIAO 1 lệnh cho user chạy ở Terminal. **Bản cài SKILL** (không có `scripts/update.command`) → version hiện tại đọc `~/.claude/kora-framework/version.json` (KHÔNG kết luận "không rõ bản" vì project thiếu file) + cập nhật = chạy lại installer `bash <(curl -fsSL …/install.command)` (tự fallback proxy + reconcile mọi project). **Sau update: VERIFY version đổi thật** (không đổi = FAIL) **+ nhắc re-upload các skill ĐỔI vào mục Skills của Cowork** (skill upload trong app KHÔNG tự cập nhật; CLAUDE.md project thì đã tự thay). **Chỉ khi** user gõ **"cập nhật" TRƠ** mới hỏi 1 câu phân biệt: *"Cập nhật ứng dụng lên bản mới, hay cập nhật tri thức/nội dung?"* |
...
| "phân tích GEO", "tăng GEO", "tối ưu AI search", "generative engine optimization", "được AI trích dẫn", "roadmap GEO", "chiến lược nội dung AI" | Confirm → **CỔNG `KORA_OPS_PW`** → `/claude-knowledge-geo-strategy` (`workflows/20-geo-strategy.md`): **HỎI NGUỒN** ([SharePoint] hỏi FOLDER→FILE · Local · vault) → import nội dung (`source: content`) → **BẮT BUỘC SPAWN Agent GEO Analyst** chấm **8 chiều GEO** (extractability·statistics·citations·authority·schema·offsite·technical·freshness — rubric `tools/geo-strategy/README.md`) ra `reports/_geo-rows.json` → `tools/geo-strategy/geo_strategy.py` sinh **scorecard + VIỆC CẦN LÀM ưu tiên + roadmap 1 năm/5 năm** (show_widget) → bước kế **[Lên chiến dịch theo roadmap]** (`claude-knowledge-campaign`) / gửi mail / xuất. |
| "kiểm tra logwork", "soát task tạo có đúng không", "kiểm tra thời gian task tháng", "validate worklog", "task đủ giờ chưa", "gợi ý lịch tạo task" | Confirm → **HỎI THÁNG** → chọn nguồn/project/(người) → **CỔNG `KORA_OPS_PW`** → `workflows/19-worklog-check.md`: quét Jira tháng đó (bổ sung field **Start date** + **Type** qua `JIRA_START_FIELD`/`JIRA_WORKTYPE_FIELD`) → `tools/worklog-validator/validate_worklog.py --validate` kiểm task **Normal** (Task/Sub-task): dueTime đúng theo est (8h/ngày, bỏ T7/CN, biên loại trừ), năng lực tháng/người, **xung đột tổng >8h/ngày** → **biểu đồ calendar timeline** (show_widget) + bảng lỗi. Rồi `--plan` **gợi ý tạo task mới** (nhập tên+est → tự đề xuất start/due/est + timeline), tùy chọn **[Tạo trên Jira]** (MCP `createJiraIssue`, đã qua cổng). *(OT/Effort ngoài cap, liệt kê riêng.)* |
...
| `tools/worklog-validator/` | `validate_worklog.py` — kiểm tra start/due/est task Normal (8h/ngày, bỏ T7/CN) + gợi ý lịch task mới + biểu đồ calendar timeline (workflow 19) |
| `tools/geo-strategy/` | `geo_strategy.py` — engine GEO (Generative Engine Optimization): chấm 8 chiều từ đánh giá per-content của Agent GEO Analyst → scorecard + action-list ưu tiên + roadmap 1 năm/5 năm + dashboard (workflow 20). Tiêu chí + nguồn ở `README.md` |
