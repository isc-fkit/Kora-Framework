...
> ⚠️ **SKILL KHỚP NHƯNG KHÔNG CÓ TRONG AVAILABLE LIST (Cowork chưa upload skill đó) → ĐỪNG thử gọi `Skill …` rồi "xin lỗi
> gọi nhầm".** Mọi skill `claude-knowledge-*` đều có **workflow tương đương trong repo** — chạy THẲNG workflow đó (qua
> `run_command`/Bash/MCP theo §⚡) mà KHÔNG cần skill được upload. CLAUDE.md + `workflows/` LUÔN nạp khi mở project nên
> luôn làm được. **Đặc biệt "cập nhật"** (`/claude-knowledge-update`): chính lúc cần update là lúc skill hay bị thiếu/cũ →
> **chạy thẳng `workflows/10-update.md`**, TUYỆT ĐỐI KHÔNG "skill không có → xin lỗi". (Vẫn confirm 1 câu cho skill gated.)
