...
| "kết nối nguồn", "connect", "thêm Jira/GitHub/GitLab/Confluence/SharePoint/Gmail" | Confirm → `/claude-knowledge-connect`: chọn **MCP/API** → nguồn (API ưu tiên **OAuth 2.0**; API vs MCP tính RIÊNG) → verify → ghi `connections:`. **Gmail → LUÔN hỏi thẳng "SMTP hay MCP", ƯU TIÊN SMTP** (tự động gửi); chỉ fallback MCP (nháp) khi user chọn rõ / không lập được App Password. **Mạng chặn SMTP → thêm [Gmail API (OAuth2)]:** flow TỰ TẠO `tools/report-mailer/.env.local` (placeholder `GMAIL_OAUTH_CLIENT_ID/SECRET` + `HTTPS_PROXY`) → user **dán 2 giá trị vào FILE** (KHÔNG qua chat) → 1 lệnh `gmail_oauth_setup.py --env <file> --write-env <file>` (đọc creds từ file → browser uỷ quyền → GHI refresh token vào CÙNG file, KHÔNG in token) → `send_report.py --transport auto` tự fallback SMTP→Gmail API. **Ghi token vào `~/.zshrc` → LUÔN `source ~/.zshrc` để lấy config MỚI** (run_command tự source mỗi lần). |
